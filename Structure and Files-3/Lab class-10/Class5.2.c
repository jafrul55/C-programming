...
int main()
{
  ...
  for(int i=0;i<k-1;i++)
  {
      for(int j=i+1;j<k;j++)
      {
          ...
          //When we want [i] number  will be small that time it will do sort for big number[i]
         if(unique_students[i].id>unique_students[j].id)  //Small to Big Mark will swap
            //You can do it for id big to small or small to big
         {
